...
class DataHandler:
    ...
    def FindPowerGenerated(self, episode, hours=24):
        """
        Returns energy in kWh for a single turbine over a given number of hours.
        Uses hourly wind data for the correct day (via Days column).
        """

        # Get the 24 hourly rows for this episode/day
        day_df = self.mast_df[self.mast_df["Days"] == episode + 2]

        if len(day_df) < hours:
            raise ValueError(f"Day {episode} has only {len(day_df)} rows, expected {hours}.")

        total_energy = 0.0

        # Loop through each hour of the day
        for h in range(hours):
            wind = day_df["ANx_80_WS_Avg"].iloc[h]
            power_kw = self.PowerFromWind(wind)   # instantaneous kW
            total_energy += power_kw              # 1 hour → kWh

        return total_energy

    def FindPowerInMaintenanceWindow(self, episode, current_step, time_skipped=3):
        """
        Returns kWh generated by a single turbine during the maintenance window.
        current_step: 1=9am, 2=12pm, 3=3pm
        time_skipped: duration of maintenance (hours)
        """
        ...
        # Extract the 24 hourly rows for this day
        day_df = self.mast_df[self.mast_df["Days"] == episode + 2] # offset due to day 1 doesn't have full day data. +1 due to 0 indexing offset, +1 due to crap data

        if len(day_df) != 24:
            raise ValueError(f"Day {episode} has {len(day_df)} rows, expected 24.")
        ...
        for h in range(start_hour, end_hour):
            wind = day_df["ANx_80_WS_Avg"].iloc[h]
            total_energy += self.PowerFromWind(wind)
